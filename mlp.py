...
mlp = MLPClassifier(hidden_layer_sizes=(256,128,64,32),activation="relu",random_state=2018,solver = 'adam',learning_rate_init=0.001,batch_size=5,momentum = 0.9).fit(X_train, y_train) #'identity', 'logistic', 'relu', 'softmax', 'tanh'].
mlp_y_pred = mlp.predict(X_test)

print('Precision score %s' % precision_score(y_test, mlp_y_pred))
print('Recall score %s' % recall_score(y_test, mlp_y_pred))
print('F1-score score %s' % f1_score(y_test, mlp_y_pred))
print('Accuracy score %s' % accuracy_score(y_test, mlp_y_pred))

output_path = valohai.outputs().path(f'mlp.pkl')
joblib.dump(mlp_y_pred, output_path)
