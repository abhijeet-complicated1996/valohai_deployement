...
print('Precision score %s' % precision_score(y_test, ada_pred))
print('Recall score %s' % recall_score(y_test, ada_pred))
print('F1-score score %s' % f1_score(y_test, ada_pred))
print('Accuracy score %s' % accuracy_score(y_test, ada_pred))

import joblib

# Save the model as a pickle in a file
output_path = valohai.outputs().path(f'adaboostclassifier.pkl')
joblib.dump(ada_classifier, output_path)